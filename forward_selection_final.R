...
complete <- read_xpt("data/extra/bpx/BPX_I.XPT") %>% 
  janitor::clean_names() %>%
...
blood_pressure <- read_xpt("data/extra/bpx/BPX_I.XPT") %>% 
  janitor::clean_names()
...
forward_predictors <- function(i_file){
  ...
  bp_data <<- read_xpt("data/extra/bpx/BPX_I.XPT") %>% 
    janitor::clean_names() %>% 
...
bp <- read_xpt("data/extra/bpx/BPX_I.XPT") %>% 
  janitor::clean_names() %>% 
  select(c(seqn, bpxsy1, bpxdi1))
...
best_forward_selection %>% 
  write_rds("data/extra/best_forward_revised.rds")

best_forward_selection %>% 
  names() %>% 
  as_tibble() %>% 
  rename(optimal_var = value) %>% 
  write_csv("data/extra/optimal_vars_revised.csv")
...
best_predictors <- read_csv("data/extra/optimal_vars_revised.csv")
